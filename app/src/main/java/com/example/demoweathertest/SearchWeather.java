...
public class SearchWeather extends AppCompatActivity {
    ...
    String key = "a3592b05a660ff8b6008a83d500636cb";//MY API-KEY
    ...
    private void getWeatherForecast(final String city) {
        ...
        if (!cacheCity.isEmpty()) {
            ...
            Request request     = new Request.Builder().url(geoURL).get().build();
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                request = new Request.Builder().url(geoURL).get().build();
            }
            // 开启子线程
